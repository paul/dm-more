      ...
      module SQL #:nodoc:
        ...
        def select_statement(query)
          ...
          unless (limit && limit > 1) || offset > 0 || qualify
            ...
            if conditions.kind_of?(AndOperation) &&
               operands.any? { |o| o.kind_of?(EqualToComparison) && o.property.unique? } &&
               !operands.any? { |o| o.kind_of?(OrOperation) }
              order = nil
              limit = nil
            end
          end
        ...
        def conditions_statement(conditions, qualify = false)
          statement, bind_values = case conditions
            when NotOperation       then negate_operation(conditions, qualify)
            when AbstractOperation  then operation_statement(conditions, qualify)
            when AbstractComparison then comparison_statement(conditions, qualify)
        ...
        def operation_statement(operation, qualify)
          ...
          operation.operands.each do |operand|
            ...
            if operand.kind_of?(OrOperation)
              statement = "(#{statement})"
            end
          ...
          join_with = operation.kind_of?(AndOperation) ? 'AND' : 'OR'
        ...
        def comparison_statement(comparison, qualify)
          ...
          operator = case comparison
            when EqualToComparison              then @negated ? inequality_operator(value) : equality_operator(value)
            when InclusionComparison            then @negated ? exclude_operator(value)    : include_operator(value)
            when RegexpComparison               then @negated ? not_regexp_operator(value) : regexp_operator(value)
            when LikeComparison                 then @negated ? unlike_operator(value)     : like_operator(value)
            when GreaterThanComparison          then @negated ? '<='                       : '>'
            when LessThanComparison             then @negated ? '>='                       : '<'
            when GreaterThanOrEqualToComparison then @negated ? '<'                        : '>='
            when LessThanOrEqualToComparison    then @negated ? '>'                        : '<='
          end
